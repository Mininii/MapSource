...
DoActions2(FP,{RotatePlayer({DisplayTextX(f_GunSendErrT,4),PlayWAVX("sound\\Misc\\Buzz.wav"),PlayWAVX("sound\\Misc\\Buzz.wav")},HumanPlayers,FP)})
...
--			PushErrorMsg("G_CA_SetSpawnX_InputData_Error")
--		end
--	end--
...
function Install_Call_G_CA()
	...
	SetCall(FP)
		CIfX(FP,CVar(FP,G_CA_TempTable[1][2],AtLeast,1,0xFF))
			...
			if TestStart == 1 then
			DoActions2(FP,{RotatePlayer({DisplayTextX(f_GunFuncT,4)},HumanPlayers,FP)})
			end
			CElseX()
			if TestStart == 1 then
				DoActions2(FP,{RotatePlayer({DisplayTextX(f_GunErrT,4),PlayWAVX("sound\\Misc\\Buzz.wav"),PlayWAVX("sound\\Misc\\Buzz.wav")},HumanPlayers,FP)})
			end
...
Trigger2X(FP, {
	...
	CommandLeastAt(BIndex,64);}, {
		RotatePlayer({
			DisplayTextX(BClear,4);
			PlayWAVX("staredit\\wav\\BossKill.ogg");
			PlayWAVX("staredit\\wav\\BossKill.ogg");
			PlayWAVX("staredit\\wav\\BossKill.ogg");
			PlayWAVX("staredit\\wav\\BossKill.ogg");}, HumanPlayers, FP);
...
function InvDisable(Cond,UnitID,Player,Loc,Name)
	...
	Trigger2X(FP, {Cond}, {
		RotatePlayer({
			...
			PlayWAVX(Wav);
			MinimapPing(1);
			DisplayTextX(txt,4);}, HumanPlayers, FP);
