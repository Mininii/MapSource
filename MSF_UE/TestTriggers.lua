function Test_LV1()
    if Limit == 1 and TestStart == 0 then
        Trigger2(FP,{ElapsedTime(AtLeast,65)},{ModifyUnitHitPoints(All,"Men",P8,64,0),RotatePlayer({DisplayTextX("\x13\x07Å×½ºÆ® ¸ðµå \x04Æ¯Àü! ¸ðµç À¯´ÖÀ» 1È¸ ²ø¾î´ç±é´Ï´Ù.\n\x13\x07Å×½ºÆ®¿¡ ÇùÁ¶ÇØÁÖ¼Å¼­ °¨»çÇÕ´Ï´Ù. \n\x13\x04Å×½ºÆ®¸Ê ÀÌ¿ë °¡´É ±â°£Àº "..YY.."³â "..MM.."¿ù "..DD.."ÀÏ "..HH.."½Ã ±îÁöÀÔ´Ï´Ù."),PlayWAVX("staredit\\wav\\button3.wav"),PlayWAVX("staredit\\wav\\button3.wav"),PlayWAVX("staredit\\wav\\button3.wav")},HumanPlayers,FP)})
...
function Test_LV2()
    if Limit == 1 and TestStart == 1 then
        ...
        local BPtr = CreateVar()
        local BPos = CreateVar()
        local BPosX = CreateVar()
        local BPosY = CreateVar()
        local CurCunit = CreateVar()
        local CurBR = CreateVar()
        local TextX = CreateVar()
        local TextY = CreateVar()
        local TextX2 = CreateVar()
        local TextY2 = CreateVar()
        local BR = CreateVar()
        local BRX = CreateVar()
        local BRY = CreateVar()
        local BRX2 = CreateVar()
        local BRY2 = CreateVar()
        local CR = CreateVar()
