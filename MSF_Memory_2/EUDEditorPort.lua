--[[
	EUDEditorPort.lua  -  EUD Editor 3(e3s) 없이 빌드하기 (MSF_UE_RE 와 같은 방식, 2026-09-16)

	예전 빌드는 EUD Editor 3 가 MSF_Mem_2.e3s 에서 빌드 데이터(DataEditor.py, ExtraDataEditor.py, RequireData,
	custom_txt.tbl, EUDEditor.eds)를 만들고 euddraft 가 그걸 플러그인으로 넣었다. 그 내용을 전부 이 폴더로 옮겼다.
	e3s 는 이제 빌드에 쓰이지 않는다. 유닛 스탯·버튼·요구조건·글은 아래 파일에서 고친다.

	  EUD Editor 가 하던 일                       지금 하는 곳
	  [DataEditor.py]  dat 패치 1182줄             EUDEditorDat.lua         -> EUDEditorInit()           첫 프레임 1회
	  [ExtraDataEditor.py] 정보창 함수 12줄        EUDEditorDat.lua         -> EUDEditorInit()           첫 프레임 1회
	  ...
	  [ExtraDataEditor.py] 버튼셋 14개             EUDEditorButtonSets.lua  -> RegisterButtonSetData()/ApplyButtonSetData()
	  [ExtraDataEditor.py] 와이어프레임 20줄       EUDEditorWireframeDefs (아래) -> EUDEditorWireframe()
	  custom_txt.tbl -> [dataDumper]               stat_txt.tbl + EUDEditorStatTxt.lua -> WriteStatTxtTbl() (컴파일 때 파일로)
	  EUDEditor.eds (APMCounter·NSQC 키 등)        eds_template.eds  (tools\build.py 가 채운다)

	■ 시점
	  EUD Editor 의 플러그인은 onPluginStart(첫 프레임, 트리거보다 먼저)와 beforeTriggerExec(매 프레임, 트리거보다 먼저)에서
	  돌았다. 여기서는 main.lua 가 StartCtrig 바로 뒤, init() 보다 앞에서 부른다.
	  - dat·정보창·풀·버튼셋은 P8(FP) 목록 앞쪽 = 첫 프레임에 OnPluginStart.lua 의 PatchArr 보다 먼저 (예전과 같은 앞뒤).
	    CtrigInitArr 에 넣지 않은 이유: EndCtrig 의 InitCtrig() 가 그 목록을 트리거 목록 "맨 끝" 에 놓는다.
	  - ★ 요구사항 오프셋 표는 Force1(P1~P4) 로 쓴다. OnPluginStart.lua 의 DoActions2(Force1,PatchArrPrsv)
	    (UnitEnable 의 오프셋 2·5)가 매 프레임 P1~P4 의 트리거 목록에서 표 위를 덮어야 예전과 같다. 표를 P8 로 쓰면
	    P1~P4 목록보다 뒤에 돌아 PatchArrPrsv 를 도로 지운다. 같은 Force1 로 먼저 선언해 두면 목록마다 표 -> PatchArrPrsv 다.
	  - 와이어프레임만 3초 뒤(main.lua 의 ElapsedTime 3 CIf 안)다. Library 의 와이어프레임 포인터(FWIRE)는 EndCtrig 가
	    목록 끝에 둔 트리거가 채우므로, 첫 프레임 맨 앞에서 ChangeWireframe 이 돌면 빈 포인터로 엉뚱한 곳에 쓴다.

	■ 검산 (2026-09-16, 옮길 때 한 번)
	  dat 1182줄 / 정보창 12줄 / 버튼셋 14개 / 요구사항 오프셋 표 5종과 풀 4260바이트 / stat_txt 1547줄 전부
	  EUD Editor 가 만든 파일과 바이트까지 같음을 확인하고 옮겼다. 헤드리스 빌드의 트리거 액션으로도 대조했다.
]]
...
local function BuildRequireData()
	...
	-- OnPluginStart.lua 의 UnitEnable 이 유닛 요구사항 오프셋을 2 또는 5 로 써서 유닛 0 스크립트 안의 자리를 빌려 쓴다
	-- (5 = 빈 스크립트 = 요구사항 없음, 2 = 배럭(111) 보유 + 애드온 짓는 중 아님 + 떠 있지 않음). 그 워드가 바뀌면
	-- 그 유닛들의 버튼 조건이 몰래 바뀌므로 멈춘다.
	for Off, Want in pairs(RequireUnitEnableWords) do
		if Pool[RequireContentBase.unit + Off + 1] ~= Want then
			error(string.format("EUDEditorRequireData: 유닛 풀 %d번 워드가 0x%04X 가 아니다 - OnPluginStart.lua 의 UnitEnable"
				.. "(오프셋 2·5)이 가리키는 조건이 바뀐다. 유닛 0 의 스크립트를 원래대로 둘 것", Off, Want))
		end
	end
...
-- ================================================================================================
-- Player = Force1 (main.lua). 이유는 맨 위 "시점" 의 ★.
function EUDEditorRequireTables(Player)
...
-- main.lua 의 선언 전용 CJump 블록(init_func) 안에서 부른다 (f_GetFileArrptr = STRCtrig 필수, Jump 구역 안에서만).
...
--[[ 첫 프레임 1회. EUD Editor 처럼 +4(배열 주소)와 +0(버튼 수)만 쓴다 - +8(connectedUnit, 영웅<->일반 연결)은
	건드리지 않았다. T 액션은 블록 안에서 만들고, 세트마다 따로 내보낸다(한 트리거 액션 수 상한). ]]
...
-- ================================================================================================
-- {유닛, 와이어프레임, 수송칸(tran), 그룹(grp)} = 그 유닛의 그림을 적힌 번호 유닛의 것으로. nil = 바꾸지 않는다.
-- EUD Editor 의 ChangeWireframe/ChangeTranframe/ChangeGrpframe = Library 의 ChangeWireframe/ChangeTranwire/ChangeGrpwire.
-- Library 제한: 수송칸은 유닛·번호 0~105, 그룹은 0~130, 와이어프레임은 0~227.
EUDEditorWireframeDefs = {
	{  0,  16,  16,  16}, -- P1 마린 (MarID[1]) = 유닛 16 의 그림
	{  1,  16,  16,  16}, -- P2 마린 (MarID[2])
	{ 99,  16,  16,  16}, -- P4 마린 (MarID[4])
	{116,  84, nil,   0},
	{117,  84, nil,   0},
	{118, 162, nil,   0},
	{184,  84},
...
-- Include_Wireframe 가 main.lua 의 init_func 안에 있어야 한다. Library 의 표는 EUD Editor 의
-- WireFrameDataEditor.eps 표와 32/64비트 모두 같다(옮길 때 대조). 매 프레임 같은 값을 다시 쓴다(Library 방식).
function EUDEditorWireframe()
	for _, D in ipairs(EUDEditorWireframeDefs) do
		if D[2] ~= nil then ChangeWireframe(FP, D[1], D[2]) end
		if D[3] ~= nil then ChangeTranwire(FP, D[1], D[3]) end
		if D[4] ~= nil then ChangeGrpwire(FP, D[1], D[4]) end
	end
end
...
-- ================================================================================================
StatTxtOutFile = "C:\\euddraft0.9.2.0\\MSF_Memory2_stat_txt.tbl" -- tools\build.py 의 STAT_TXT_OUT 과 같아야 한다

local function U16(S, P) return S:byte(P) + S:byte(P + 1) * 256 end

-- UTF-8 로 적은 줄 -> CP949 바이트. 널 문자로 나뉜 조각마다 바꾸고 널은 그대로 둔다(버튼 툴팁의 \0 등).
...
function WriteStatTxtTbl()
	local Path = Curdir .. "MapSource\\MSF_Memory_2\\" .. StatTxtBaseFile
