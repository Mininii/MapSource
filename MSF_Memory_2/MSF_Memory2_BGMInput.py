"""MSF_Memory_2 음원을 빌드 때 맵에 넣는 euddraft 플러그인 (MSF_UE_RE_BGMInput.py 와 같다).

음원 원본은 맵이 아니라 C:/euddraft0.9.2.0/MSF_Memory2_BGM/ 에 있다 (tools/split_map.py 가 원본 맵에서
꺼내 둔다). 그 폴더의 .ogg/.wav 를 전부 원래 이름 'staredit\\wav\\<파일명>' 으로 출력 맵의 MPQ 에
넣는다. 트리거의 PlayWAV 문자열과 chk 의 WAV 목록이 이 이름을 가리킨다.

tools/build.py 가 이 파일을 eds 옆에 복사하고 [MSF_Memory2_BGMInput.py] 섹션으로 부른다.
섹션의 `Path : <폴더>` 로 음원 폴더를 바꿀 수 있다. 음원을 더하려면 폴더에 넣고 다시 빌드한다.
"""
from eudplib import *
import os

BGM_DIR = "C:/euddraft0.9.2.0/MSF_Memory2_BGM/"
SOUND_EXT = (".ogg", ".wav")
...
def Exec_OggFile():
    ...
    for filename in names:
        ...
        total += len(data)
    print("[MSF_Memory2_BGMInput] %s : %d files, %.1f MB" % (src, len(names), total / 1e6))
