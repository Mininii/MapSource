...
for i = 0, 6 do
	...
	for j = 1, 4 do
		...
		f_SaveCp()
		DoActions2(FP, {RotatePlayer({PlayWAVX("sound\\Terran\\Medic\\TMdPss06.wav"),PlayWAVX("sound\\Terran\\Medic\\TMdPss06.wav")},HumanPlayers, FP)})
...
NJumpEnd(FP, EnemyArea)
	...
	for j,k in pairs(UnitPointArr) do
		Trigger2X(FP, {CVX(HeroIndex, k[1], 0xFF)}, {SetScore(Force1, Add, k[2], Kills),RotatePlayer({PlayWAVX("staredit\\wav\\Computer Beep.wav"),PlayWAVX("staredit\\wav\\Computer Beep.wav"),DisplayTextX(StrDesignX("\x04적 유닛 \x03"..k[3].." \x07처치! \x1F+ "..k[2].." P t s"), 4)}, HumanPlayers, FP)},{preserved})
	end
	f_LoadCp()
	CIfEnd()
EXCC_ClearCalc()
