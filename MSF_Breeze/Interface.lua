...
CIfX(FP,Never()) -- 상위플레이어 단락 시작
	...
	for i = 0, 6 do
	TriggerX(FP, {CD(LimitT[i+1],1),Bring(i, AtLeast, 1, 15, 76)}, {SetCD(UTAGECcode,1),RotatePlayer({DisplayTextX(StrDesignX("\x07연회장 모드\x04가 활성화되었습니다.").."\n"..StrDesignX("기본 방어력 255 제공, 공업 제한 삭제, 영작포인트 "..EVFPtsMul.."배, 건작 패턴 다수 변경"), 4),PlayWAVX("staredit\\wav\\GameStart.ogg"),PlayWAVX("staredit\\wav\\GameStart.ogg"),PlayWAVX("staredit\\wav\\GameStart.ogg")}, HumanPlayers, FP),})
	end
