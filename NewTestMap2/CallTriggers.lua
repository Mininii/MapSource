function Install_CallTriggers()
	...
	local TempReadW = CreateWar(FP)

	local CheatTestJump = def_sIndex()
	CJumpEnd(FP, CheatTestJump)
	NIf(FP,{TTNWar(CTPEXP,AtLeast,CTTotalExp),CV(CTPLevel,49999,AtMost)},{AddV(CTPLevel,1)}) -- 경험치 치팅 검사
	ConvertLArr(FP, CTLIndex, _Sub(CTPLevel, 1), 8)
	f_LRead(FP, LArrX({EXPArr},CTLIndex), TempReadW, nil, 1)
	f_LAdd(FP, CTTotalExp, CTTotalExp, TempReadW)
	
	ConvertLArr(FP, CTLIndex, _Sub(CTPLevel, 2), 8)
	f_LRead(FP, LArrX({EXPArr},CTLIndex), TempReadW, nil, 1)
	f_LAdd(FP, CTCurEXP, CTCurEXP, TempReadW)
					...
					CIf(FP,{CV(G_PushBtnm,3,AtLeast),CV(G_PushBtnm,8,AtMost)},{}) -- 승급
						...
						CElseX()
							...
							CIfX(FP,{TBring(GCP, AtLeast, 1, GetUID, _Add(GCP,160))})--승급하기
								...
								NJumpX(FP,ClassUpErrJump,{CV(G_PushBtnm,5),CV(GetEXPData,CS_EXPLimit,AtLeast)},{PlayWAV("sound\\Misc\\PError.WAV"),DisplayText(StrDesignX("\x08ERROR \x04: 더 이상 경험치 획득량을 올릴 수 없습니다."), 4),SetCp(FP)})
								...
								CIfX(FP,{TTNWar(GetCreditData,AtLeast,"1000000")})
									...
									f_Read(FP,_Add(G_Btnptr,10),CPos)
									Convert_CPosXY()
									Simple_SetLocX(FP, 86, CPosX, CPosY, CPosX, CPosY,{CreateUnitWithProperties(10,82,87,FP,{hallucinated = false}),GiveUnits(All, 82, FP, 64, P9),KillUnit(82, P9)})
