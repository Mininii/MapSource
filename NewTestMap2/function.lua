...
function SetUnitsDatX(UnitID,Property)
	if type(UnitID) == "string" then
		UnitID = ParseUnit(UnitID) -- ï¿½ï¿½Æ®ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½Ì¸ï¿½ ï¿½Ô·Â°ï¿½ï¿½ï¿½
	end
	...
	if type(Property)~= "table" then
		...
		for j,k in pairs(Property) do
			if j=="MinCost" then
				PatchInsert(SetMemoryW(0x663888 + (UnitID *2),SetTo,k)) -- ï¿½Ì³×¶ï¿½
			...
			elseif j=="GasCost"  then
				PatchInsert(SetMemoryW(0x65FD00 + (UnitID *2),SetTo,k)) -- ï¿½ï¿½ï¿½ï¿½
			elseif j=="BuildTime"  then
				PatchInsert(SetMemoryW(0x660428 + (UnitID *2),SetTo,k)) -- ï¿½ï¿½ï¿½ï¿½Óµï¿?
			elseif j=="SuppCost"  then
				PatchInsert(SetMemoryB(0x663CE8 + UnitID,SetTo,k*2)) -- ï¿½ï¿½ï¿½ï¿½
			elseif j=="HP"  then
			...
			elseif j=="Armor" then
				PatchInsert(SetMemoryB(0x65FEC8 + (UnitID),SetTo,k)) -- ï¿½ï¿½ï¿½ï¿½
			elseif j=="GroupFlag" then
				PatchInsert(SetMemoryB(0x6637A0 + (UnitID),SetTo,k)) -- ï¿½×·ï¿½
			elseif j=="Height" then
				PatchInsert(SetMemoryB(0x663150 + (UnitID),SetTo,k)) -- ï¿½Ç¼ï¿½Å©ï¿½ï¿½
			elseif j=="BdDimX" then
				PatchInsert(SetMemoryX(0x662860 + (UnitID*4),SetTo,k,0xFFFF)) -- ï¿½Ç¼ï¿½Å©ï¿½ï¿½
			elseif j=="BdDimY" then
				PatchInsert(SetMemoryX(0x662860 + (UnitID*4),SetTo,k*65536,0xFFFF0000)) -- ï¿½Ç¼ï¿½Å©ï¿½ï¿½
			elseif j=="SizeL" then
			...
			elseif j=="SuppProv" then
				PatchInsert(SetMemoryB(0x6646C8+UnitID,SetTo,Property.SuppProv)) -- ï¿½ï¿½ï¿½Ã°ï¿½ï¿½Þ·ï¿½
...
function SetWeaponsDatX(WepID,Property)
	if type(Property)~= "table" then
		...
		for j,k in pairs(Property) do
			if j=="DmgBase" then
				PatchInsert(SetMemoryW(0x656EB0+(WepID *2),SetTo,k)) -- ï¿½ï¿½ï¿½Ý·ï¿½
			elseif j=="DmgFactor" then
				PatchInsert(SetMemoryW(0x657678+(WepID *2),SetTo,k)) -- ï¿½ß°ï¿½ï¿½ï¿½ï¿½Ý·ï¿½
			elseif j=="Cooldown" then
				PatchInsert(SetMemoryB(0x656FB8+(WepID *1),SetTo,k)) -- ï¿½ï¿½ï¿½ï¿½
			elseif j=="Splash" then
				if type(k)=="boolean" then
					if k == true then
						PatchInsert(SetMemoryB(0x6566F8+WepID,SetTo,3)) -- ï¿½ï¿½ï¿½ï¿½Å¸ï¿½ï¿½(ï¿½Ï¹ï¿½ï¿½ï¿½)
					else
						PatchInsert(SetMemoryB(0x6566F8+WepID,SetTo,1)) -- ï¿½ï¿½ï¿½ï¿½Å¸ï¿½ï¿½(ï¿½ï¿½ï¿½Ã¾ï¿½ï¿½ï¿½)
					end
				elseif type(k)~="table" or #k~=3 then
					PushErrorMsg("Splash Inputdata Error")
				else
					PatchInsert(SetMemoryB(0x6566F8+WepID,SetTo,3)) -- ï¿½ï¿½ï¿½ï¿½Å¸ï¿½ï¿½(ï¿½Ï¹ï¿½ï¿½ï¿½)
					PatchInsert(SetMemoryW(0x656888+(WepID*2),SetTo,k[1])) --ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½
					PatchInsert(SetMemoryW(0x6570C8+(WepID*2),SetTo,k[2])) --ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½
					PatchInsert(SetMemoryW(0x657780+(WepID*2),SetTo,k[3])) --ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½
				end
			elseif j=="RangeMin" then
				PatchInsert(SetMemory(0x656A18+(WepID *4),SetTo,k)) -- ï¿½ï¿½Å¸ï¿? ï¿½Ö¼ï¿½
			elseif j=="RangeMax" then
				PatchInsert(SetMemory(0x657470+(WepID *4),SetTo,k)) -- ï¿½ï¿½Å¸ï¿? ï¿½Ö´ï¿½
			...
			elseif j=="ObjectNum" then
				PatchInsert(SetMemoryB(0x6564E0+WepID,SetTo,k)) -- ï¿½ï¿½ï¿½ï¿½Ã¼ï¿½ï¿½
			elseif j=="IconType" then
				PatchInsert(SetMemoryW(0x656780+(WepID *2),SetTo,k)) -- ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
			...
			elseif j== "WepName" then
				PatchInsert(SetMemoryW(0x6572E0+(WepID *2),SetTo,k)) -- ï¿½Ì¸ï¿½
...
function PopLevelUnit()
	...
	SetUnitsDatX(LevelUnitArr[#LevelUnitArr][2], {DefUpType=60}) -- ï¿½Ö°ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½È­È®ï¿½ï¿½ ï¿½ï¿½ï¿½ß±ï¿½
...
function CIfKeyFunc(CP,Key,ContentStr,DisContentStr,Conditions,CondActions,DisCondActions)
	...
	CTrigger(FP,{CDeaths(FP,AtMost,0,ShopKey[CP+1]),Conditions},{SetCDeaths(FP,SetTo,1,ShopKey[CP+1]),CondActions},{preserved})	-- ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿?
	CTrigger(FP,{CDeaths(FP,AtMost,0,ShopKey[CP+1]),LocalPlayerID(CP)},{print_utf8(12,0,DisContentStr)},{preserved})
	CTrigger(FP,{CDeaths(FP,AtMost,0,ShopKey[CP+1])},{SetCDeaths(FP,SetTo,1,ShopKey[CP+1]),DisCondActions},{preserved})	-- ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿?
end
function KeyFunc(CP,Key,ContentArgs) --{{1ï¿½ï¿½ï¿½ï¿½ï¿½Ç¹è¿­,ï¿½×¼Ç¹è¿­,ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ø½ï¿½Æ?},...}
	CIf(FP,{MSQC_KeyInput(CP, Key)})
...
function CIfBtnFunc(CP,ID,ContentStr,DisContentStr,Conditions,CondActions,DisCondActions)
	...
	CTrigger(FP,{CDeaths(FP,AtMost,0,ShopSw[CP+1]),Conditions},{SetCDeaths(FP,SetTo,1,ShopSw[CP+1]),CondActions},{preserved})	-- ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿?
	CTrigger(FP,{CDeaths(FP,AtMost,0,ShopSw[CP+1]),LocalPlayerID(CP)},{print_utf8(12,0,DisContentStr)},{preserved})
	CTrigger(FP,{CDeaths(FP,AtMost,0,ShopSw[CP+1])},{SetCDeaths(FP,SetTo,1,ShopSw[CP+1]),DisCondActions},{preserved})	-- ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿?
end
...
function MSQC_KeySet(KeyName,DeathUnit) -- Å°ï¿½Î½Ä¿ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿?
	MSQC_KeyArr[KeyName] = DeathUnit
end

function MSQC_KeyInput(Player,KeyName) -- Å°ï¿½Î½Ä¿ï¿½ ï¿½ï¿½ï¿½ï¿½
