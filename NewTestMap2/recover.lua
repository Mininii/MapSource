...
--Cost_FAcc = CreateCostDataFile(30, function(n) return 150000000+((n-1)*50000000) end, "FAcc")--유료전용 촉진
--Cost_FAcc2 = CreateCostDataFile(30, function(n) return 300000000+((n-1)*100000000) end, "FAcc2")--무료전용 촉진
--Cost_FBrSh2 = CreateCostDataFile(1000,function(n) return 5000000+((n-1)*(n*1)*(n*12)) end,"FBrSh2")--수호의 보석
--Cost_FMEPer = CreateCostDataFile(350,function(n) return 4000000+((n-1)*(n*50)*n) end,"FMEPer2")--진 궁극
--Cost_FMinMax = CreateCostDataFile(1000,function(n) return n*5000000 end,"FMinMax")--채굴(광산최대치 퍼센트증가)
--
...
--Cost_FMinMax = CreateCostDataFile(1000,function(n) return n*5000000 end,"FMinMax_Test")--채굴(광산최대치 퍼센트증가)
--Cost_FBrSh2 = CreateCostDataFile(1000,function(n) return 100+((n-1)*(n*3)*n) end,"FBrSh2_Test")--수호의 보석
...
function PopSCAData()
	...
	io.close(Subtitle)
	Subtitle = io.open(FileDirectory .. "Data" .. ".txt", "wb")

	for j,k in pairs(DataTable) do
		Subtitle:write(k.."\n")
	end
...
--    if tarr[k[2]]==nil then
--        tarr[k[2]]=k[1]
--    end
--end
--for j,k in pairs(arr2) do
--    tarr[k[2]]=true
--end


function Create_SCA_AddSubData(SCAID,SCID,DataName,Value)
    if type(DataName) ~= "table" or type(Value) ~= "table"  then  error("Data_Type_Error") end
    for j,k in pairs(DataName) do
        table.insert(DataTable,SCAID.."	"..SCID.."	"..k.."	"..Value[j].."	0	3")
    end
...
function comb(a,b) return math.floor(fac(a)/(fac(b)*fac(a-b))) end
--print(po(6, 10, 0.5))

function po(x,n,p)
    return comb(n,x) * p * (p^x) * (1 - p) * (p^(n-x))
end
to = 0
for i = 1,10 do
    to = to+po(i, 10, 0.5)
