...
| `CHANGELOG.md` | 판별 변경 기록 (플러그인 판 1.3, CtrigAsm 판 1.3) |
| `tests/test_snqc.py` | 플러그인 오프라인 컴파일 시험 (eudplib 0.76.14 / 0.81) |
| `tests/t_snqc_emu.py`, `tests/emu.py` | 플러그인 에뮬레이터 시험 - 보내기 → 턴 → 받기 (키 칸, 합치기 등, eudplib 0.76.14 / 0.81) |
...
| | 플러그인 판 1.3 | CtrigAsm 판 1.3 |
| --- | --- | --- |
| 문법·컴파일 | eudplib 0.76.14·**0.81 (1.3)** 오프라인 컴파일 (theSeed·DPS 설정) + 에뮬레이터 시험 (1.3), **MSF_UE_RE 풀 빌드 (1.2)** | **MSF_UE_RE 실제 tepc 컴파일 (1.3)** + 가짜 환경 실행 |
...
| 남은 확인 | 1.3 인게임 전부 (1.3 = eudplib 0.81 키 배열·합치기 판정 수정, 에뮬레이터 시험까지), 멀티(디싱크), 합치기(턴이 여러 사이클인 방), 버퍼 한계, 0.81 빌드. 1.1: DPS강화하기 인게임 정상 | 멀티(디싱크) - DESIGN.md 7절 5·9. MSF_UE_RE 는 지금 이 판(`SNQC_LUA`)으로 되어 있다 |
