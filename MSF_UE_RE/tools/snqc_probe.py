...
메모리 리더는 DPS_Enhance/tools/debugbridge/dbg_reader.py 를 쓴다 (SCR_DB 런처와 같은 방식).
"""
import argparse
import os
import struct
...
sys.path.insert(0, os.path.join(DOCS, "DPS_Enhance", "tools", "debugbridge"))
...
def show(proc, delta):
    ...
    for c in range(nch):
        ...
        print("%3d  %6d  %04X  %5d  %5d  %5d  %08X  %4d  P%-2d %3d  %08X  %08X  %4d %4d  %3d  %s"
              % (c + 1, e[0], alpha, e[2], e[3], e[4], e[5], e[6] & 0xFFFF, (ownord & 0xFF) + 1,
                 (ownord >> 8) & 0xFF, e[8], e[9], pos & 0xFFFF, pos >> 16, (e[11] >> 8) & 0xFF, scr))
